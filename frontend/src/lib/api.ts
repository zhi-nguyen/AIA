...
export async function getChatSessions(): Promise<{ status: string; sessions: ChatSession[] }> {
  const res = await fetchWithAuth(`${API_BASE_URL}/chat/sessions`);
  if (!res.ok) throw new Error(`HTTP ${res.status}`);
  return res.json();
}

export async function createChatSession(title: string = "Cuộc trò chuyện mới", is_temporary: boolean = false): Promise<{ status: string; session_id: string }> {
  const res = await fetchWithAuth(`${API_BASE_URL}/chat/sessions`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ title, is_temporary })
  });
  if (!res.ok) throw new Error(`HTTP ${res.status}`);
  return res.json();
}

export async function getSessionMessages(sessionId: string): Promise<{ status: string; messages: ChatMessage[] }> {
  const res = await fetchWithAuth(`${API_BASE_URL}/chat/sessions/${sessionId}/messages`);
  if (!res.ok) throw new Error(`HTTP ${res.status}`);
  return res.json();
}

export async function deleteChatSession(sessionId: string): Promise<{ status: string; deleted: boolean }> {
  const res = await fetchWithAuth(`${API_BASE_URL}/chat/sessions/${sessionId}`, {
    method: "DELETE"
  });
  if (!res.ok) throw new Error(`HTTP ${res.status}`);
  return res.json();
